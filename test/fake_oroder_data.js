...
const {
  AWS_DB_HOST, AWS_DB_USER, AWS_DB_PASSWORD, AWS_DB_DATABASE,
} = process.env;
...
async function fakeOrderGenerator(max_range) {
  for (let i = 0; i < max_range; i++) {
    const user_id = random_generator(1, 5);
    const total = random_generator(100, 1000);
    // orders_insert(user_id, total)
    insert_set.push([user_id, total]);
  }
  await query('TRUNCATE TABLE orders');
  const fake_orders_query = 'INSERT INTO orders (user_id, total) VALUES ?';
